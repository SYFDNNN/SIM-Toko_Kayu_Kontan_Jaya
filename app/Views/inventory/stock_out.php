...
<head>
  ...
  <title>Stock Out — Toko Kayu Kontan Jaya</title>
  ...
  <style>
    * { font-family: 'Plus Jakarta Sans', sans-serif; box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .form-card-header { padding: 20px 24px; border-bottom: 1px solid var(--border); background: linear-gradient(135deg, #FEF2F2 0%, #FEE2E2 100%); display: flex; align-items: center; gap: 12px; }
    .form-card-icon { width: 40px; height: 40px; border-radius: 12px; background: #DC2626; display: flex; align-items: center; justify-content: center; }
    ...
    .form-control:focus { border-color: #DC2626; background: white; box-shadow: 0 0 0 3px rgba(220,38,38,0.1); }
    .form-control::placeholder { color: var(--muted); }
    select.form-control { cursor: pointer; }
    textarea.form-control { resize: vertical; min-height: 90px; }
    .form-hint { font-size: 11px; color: var(--muted); margin-top: 5px; }

    /* Stock preview — red variant */
    .stock-preview { background: #FEF2F2; border: 1.5px solid #FECACA; border-radius: 12px; padding: 14px 16px; margin-bottom: 20px; display: none; }
    ...
    .stock-preview-label { color: #991B1B; font-weight: 500; }
    .stock-preview-val { font-weight: 700; color: #DC2626; }
    .warn-text { font-size: 11px; color: #DC2626; font-weight: 600; margin-top: 8px; display: none; }
    .warn-text.show { display: block; }

    .btn-danger { background: #DC2626; color: white; font-size: 14px; font-weight: 700; padding: 11px 24px; border-radius: 10px; border: none; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; transition: all 200ms; font-family: inherit; }
    .btn-danger:hover { background: #B91C1C; transform: translateY(-2px); box-shadow: 0 8px 24px rgba(220,38,38,0.3); }
...
<div id="main">
  ...
  <div class="content">
    ...
    <div class="page-header">
      <h1 style="font-size:22px;font-weight:800;color:var(--text);letter-spacing:-0.02em;">Stock Out</h1>
      <p style="font-size:13px;color:var(--muted);margin-top:3px;">Catat pengeluaran stok dari gudang</p>
    </div>
    ...
    <div class="form-card">
      <div class="form-card-header">
        ...
        <div>
          <div style="font-size:15px;font-weight:700;color:#991B1B;">Catat Stok Keluar</div>
          <div style="font-size:12px;color:#dc2626;margin-top:1px;">Isi form di bawah untuk mencatat pengeluaran barang</div>
        </div>
      </div>
      <div class="form-card-body">
        <form action="<?= base_url('inventori/stock-out') ?>" method="POST">
          <?= csrf_field() ?>
          ...
          <div class="stock-preview" id="stockPreview">
            ...
            <div class="stock-preview-row">
              <span class="stock-preview-label">Qty Keluar</span>
              <span class="stock-preview-val" id="prevQty">-0</span>
            </div>
            <div style="border-top:1px dashed #FECACA;margin:8px 0;"></div>
            <div class="stock-preview-row">
              <span class="stock-preview-label" style="font-weight:700;">Sisa Stok</span>
              <span class="stock-preview-val" id="prevAfter" style="font-size:16px;">—</span>
            </div>
            <div class="warn-text" id="warnText">⚠ Stok tidak mencukupi!</div>
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            <div class="form-group">
              ...
              <div class="form-hint">Jumlah barang yang dikeluarkan</div>
            </div>
            <div class="form-group">
              <label class="form-label">Alasan Pengeluaran</label>
              <select name="reason" class="form-control">
                <option value="">— Pilih Alasan —</option>
                <option value="adjustment" <?= old('reason')=='adjustment'?'selected':'' ?>>Penyesuaian Stok</option>
                <option value="damaged" <?= old('reason')=='damaged'?'selected':'' ?>>Barang Rusak / Cacat</option>
                <option value="expired" <?= old('reason')=='expired'?'selected':'' ?>>Kadaluarsa</option>
                <option value="transfer" <?= old('reason')=='transfer'?'selected':'' ?>>Transfer Gudang</option>
          ...
          <div class="form-group">
            ...
            <input type="text" name="reference_no" class="form-control"
              placeholder="cth: ADJ-2024-001" value="<?= old('reference_no') ?>">
          </div>

          <div class="form-group">
            <label class="form-label">Catatan</label>
            <textarea name="notes" class="form-control" placeholder="Keterangan tambahan..."><?= old('notes') ?></textarea>
          </div>
...
function updateStockPreview() {
  ...
  const remaining = curStock - qty;
  document.getElementById('prevStock').textContent = curStock + ' ' + unit;
  document.getElementById('prevQty').textContent = '-' + qty + ' ' + unit;
  document.getElementById('prevAfter').textContent = remaining + ' ' + unit;
  document.getElementById('prevAfter').style.color = remaining < 0 ? '#EF4444' : '#DC2626';
  document.getElementById('warnText').classList.toggle('show', remaining < 0);
